...
const personData = JSON.parse(dataFromFile);
...
function fullNameOfAllPeopleOver22Under45FirstOrLastWithL(listOfPeople) {
  ...
  const firstOrLastNameStartWithL = youngerThan43OlderThan22.filter(
    (person) => person.first_name.charAt(0) === 'L' || person.last_name.charAt(0) === 'L',
  );
...
function peopleWithGovEmail(listOfPeople) {
  const arrayOfPeopleWtihGovEmail = listOfPeople.filter(
    (person) => person.email.slice(person.email.length - 3) === 'gov'
      || person.email.includes('.gov.'),
  );
  return arrayOfPeopleWtihGovEmail.map(
    (person) => `${person.first_name} ${person.last_name} ${person.ip_address}`,
  );
}
...
module.exports = {
  femalesCount,
  fullNameOfAllPeopleOver35,
  ageSum,
  averageAge,
  fullNameOfAllPeopleOver22Under45FirstOrLastWithL,
  peopleWithGovEmail,
};
